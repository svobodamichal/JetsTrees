 ...
 */
void unfold_data_test(const char* dataFile,
                 const char* responseFile,
                 const char* outDir,
                 int nIter /* = kBayesIterDefault */,
                const char* method = "Bayes")
{

    std::string m(method);
    std::cout << ">>> Unfolding data with = " << m << std::endl;
  gStyle->SetOptStat(0);
  EnsureDir(outDir);
  ...
  for (size_t iR = 0; iR < kRadii.size(); ++iR) {
    ...
    for (size_t iC = 0; iC < kCentralities.size(); ++iC) {
      ...
      for (int ic = 0; ic < kNPtLeadCuts; ++ic) {
        ...
        // Create backfolded spectrum (refolded)
              TH1D* hBackfolded = (TH1D*)response->ApplyToTruth(hUnfoldedTruthBins);
              hBackfolded->SetName(Form("BackfoldedRecoBins_%s", tag.c_str()));
              hBackfolded->SetDirectory(0);

            cout << "    backfolded integral (reco bins) = "
            << hBackfolded->Integral(0,-1) << endl;
        ...
        if (m=="SVD") {
          cout << "    running RooUnfoldSvd\n";

          //const int kRegValues[] = {2, 3, 4, 5, 6, 7, 8};
          const int kRegValues = 5;
          //const int nSVD = sizeof(kRegValues)/sizeof(kRegValues[0]);

          //vector<TH1D*> unfoldedSVD(nSVD, nullptr);
          
            RooUnfoldSvd unfoldSVD(response, hMeasData, kRegValues);

            TH1D* hUnfoldedRecoBins_SVD = (TH1D*)unfoldSVD.Hunfold(); // reco-binning
            hUnfoldedRecoBins_SVD->SetName(Form("UnfoldedRecoBins_SVD_%s", tag.c_str()));
            hUnfoldedRecoBins_SVD->SetDirectory(0);

            // Rebin to truth binning (this is what you'll use later)
            TH1D* hUnfoldedTruthBins_SVD = (TH1D*)hUnfoldedRecoBins_SVD->Rebin(
                nbins_truth,
                Form("UnfoldedTruthBins_SVD_%s", tag.c_str()),
                bin_truth_edges);
            hUnfoldedTruthBins_SVD->SetDirectory(0);

            cout << "    unfolded integral (truth bins) = "
                << hUnfoldedTruthBins_SVD->Integral(0,-1) << endl;


            // Create backfolded spectrum (refolded)
              TH1D* hBackfolded_SVD = (TH1D*)response->ApplyToTruth(hUnfoldedTruthBins_SVD);
              hBackfolded_SVD->SetName(Form("BackfoldedRecoBins_SVD_%s", tag.c_str()));
              hBackfolded_SVD->SetDirectory(0);

            cout << "    backfolded integral (reco bins) = "
            << hBackfolded_SVD->Integral(0,-1) << endl;
            ...
            hMeasData->Write("hMeasData");
            hUnfoldedRecoBins_SVD->Write("hUnfoldedRecoBins");
            hUnfoldedTruthBins_SVD->Write("hUnfoldedTruthBins");
            hBackfolded_SVD->Write("hBackfolded");

            // optional: save covariance matrix
            TMatrixD cov = unfoldSVD.Eunfold(RooUnfold::kCovariance);
            cov.Write("covariance");
            ...
            // cleanup
            delete hMeasData;
            delete hUnfoldedRecoBins_SVD;
            delete hUnfoldedTruthBins_SVD;
            delete hBackfolded_SVD;
